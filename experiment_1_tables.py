...
def get_stats_for_metric(df_raw, methods, ref_method, metric_name, metric_col):
    ...
    for method in methods:
        ...
        if method != ref_method:
            ...
            if ref_col_name not in df_raw.columns or col_full_name not in df_raw.columns:
                p_val_str = "N/A"
            else:
...
def create_metric_table(
    ...
    all_stats = {
        "'Nice' Distributions": get_stats_for_metric(data_groups['nice'], methods, ref_method, metric_name, metric_col),
        "'Bimodal' Distribution": get_stats_for_metric(data_groups['bimodal'], methods, ref_method, metric_name, metric_col),
    }
    if include_hard_dist:
        all_stats["'Hard' Distributions"] = get_stats_for_metric(data_groups['hard'], methods, ref_method, metric_name, metric_col)
        group_names.append("'Hard' Distributions")

    # 2. Build the DataFrame
    # First, get the mean scores
    df_mean = pd.DataFrame.from_dict({group: {m: s['mean'] for m, s in stats.items()} 
                                    for group, stats in all_stats.items()}, orient='index').T
...
def print_appendix_tables(df_raw, method_order, latex_path=None):
    ...
    def create_table(metric_cols, metric_name, file_suffix, float_format):
        ...
        df_metric_latex = df_metric.rename(columns={
        ...
        if latex_path:
            ...
            try:
                ...
                df_latex.columns = [m.replace(f"_{metric_name}", "") for m in df_latex.columns]
    ...
    lscv_cols = [f"{m}_lscv_score" for m in method_order if f"{m}_lscv_score" in df_summary.columns]
    if lscv_cols:
        create_table(lscv_cols, "lscv_score", "a_lscv", ".4f")
    
    ise_cols = [f"{m}_ise_score" for m in method_order if f"{m}_ise_score" in df_summary.columns]
    if ise_cols:
        create_table(ise_cols, "ise_score", "b_ise", ".4f")

    time_cols = [f"{m}_comp_time" for m in method_order if f"{m}_comp_time" in df_summary.columns]
    if time_cols:
    ...
    if 'BETA_ROT_is_fallback' in df_summary.columns:
        ...
        if latex_path:
            ...
            try:
                ...
                df_latex.rename(columns={'BETA_ROT_is_fallback': r'\rott'}, inplace=True)
                df_latex[r'\rott'] = df_latex[r'\rott'].apply(lambda x: f"{x*100:.2f}\\%" if pd.notna(x) else "-")
...
if __name__ == "__main__":
    ...
    if raw_data is not None:
        ...
        create_metric_table(
            data_groups, method_order, "BETA_ROT", 
            "LSCV", "lscv_score", 
